...
function phone(string, num) {
  ...
  const phonebookObj = splitStrArr.map((str) => {
    const { nameReg, phoneReg, addressReg } = regexObj;
    let currentStr = str;
    const name = currentStr.match(nameReg)[1];
    const phone = currentStr.match(phoneReg)[1].split("+")[1];
    currentStr = currentStr.replace(phoneReg, ``).replace(nameReg, ``);
    const address = currentStr
      .replace(addressReg, " ")
      .split(" ")
      .filter((item) => item)
      .join(" ");
  ...
  let currentPhoneObj = phonebookObj.filter((item) => item.phone === num);

  if (currentPhoneObj.length > 1) {
    return `Error => Too many people: ${num}`;
  } else if (!currentPhoneObj.length) {
    return `Error => Not found: ${num}`;
  }

  currentPhoneObj = currentPhoneObj[0];

  return `Phone => ${currentPhoneObj.phone}, Name => ${currentPhoneObj.name}, Address => ${currentPhoneObj.address}`;
}

console.log(phone(dr, "1-541-754-3010")); // "Phone => 48-421-674-8974, Name => Anastasia, Address => Via Quirinal Roma"
